"""
Compares single-threaded vs max-thread alpha-beta depth performance.
...
def run_comparison(max_depth, max_positions, single_threads, multi_threads):
    print(f"[calibrate_alpha_beta_depth] running single-thread  (OMP_NUM_THREADS={single_threads}) ...")
    single = spawn_worker(single_threads, max_depth, max_positions)

    print(f"[calibrate_alpha_beta_depth] running multi-thread   (OMP_NUM_THREADS={multi_threads}) ...")
