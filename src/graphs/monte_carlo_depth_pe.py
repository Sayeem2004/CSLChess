...
def parse_depth_file(path):
    """
    Parse calibrate_monte_carlo_depth output.
    Returns:
...
def plot_pe(schemas, data, depth, input_path, output_dir):
    ...
    x      = np.arange(n_cols)
    width  = 0.55
    colors = plt.cm.Greens(np.linspace(0.35, 0.85, n_cols))
    ...
    ax.set_title(
        f"MCTS Parallel Efficiency — Depth {depth}\n"
...
if __name__ == "__main__":
    default_input = os.path.join(
        ...
        "data", "outputs", "calibrate", "monte-carlo-depth", "monte-carlo-depth.txt"
    )
    default_output = os.path.join(DATA_DIR, "graphs")

    parser = argparse.ArgumentParser()
    parser.add_argument("--input",  default=default_input,
                        help="Path to calibrate_monte_carlo_depth output file")
