...
from utils.move_cycles import cpp_best_move_cycles, stockfish_best_move_cycles, csl_best_move_cycles
...
def engine_best_move(algorithm: str, engine, board: chess.Board, budget_type: str, budget: int) -> chess.Move | None:
    ...
    if budget_type == "cycles":
        return csl_best_move_cycles(engine, board, budget) if is_csl else cpp_best_move_cycles(engine, board, budget)
